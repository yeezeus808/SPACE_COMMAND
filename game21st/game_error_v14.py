...
G_CONSTANT      = 20_000   # gravitational constant (tuned for fun)
BLACK_HOLE_MASS = 1.0      # relative mass multiplier
BH_MAX_ACCEL    = 2.0      # cap gravity acceleration per frame (prevents inescapable pull)
BH_SPEED_REDUCTION = 1.0   # ball speed multiplier on black hole exit (0.1 = very slow, 1.0 = no change)
...
# AI settings
AI_SETTINGS = {
    "easy":   {"speed": 4,  "delay": 20, "error": 60},
    "medium": {"speed": 8,  "delay": 10, "error": 30},
    "hard":   {"speed": 12, "delay": 2,  "error": 8},
}
...
class Stars:
    def __init__(self, n=150):
        ...
        self.speeds    = [random.uniform(0.2, 0.8) for _ in range(n)]
    def update(self):
...
class AsteroidBelt:
    """Conveyor belt net — scrolls, cycles density, and lets fast balls punch through."""

    def __init__(self, difficulty=None):
        self.scroll_speed  = BELT_SCROLL_SPEED.get(difficulty, 1.0)
        self.scroll_offset = 0.0      # current scroll position
        self.density_timer = 0        # tracks dense/sparse cycle
        self.asteroids     = []
        self._spawn_asteroids()

    def _spawn_asteroids(self):
        """Spawn asteroids across a doubled height so wrapping looks seamless."""
        self.asteroids = []
        cx = WIDTH // 2
        # Spawn across 2x screen height for smooth wrapping
        for _ in range(42):
            ...
            self.asteroids.append({
                "x": x, "y": y, "r": r,
                "base_y": y,              # original Y for density calculations
                "col": (shade, shade//2, shade//3),
                "visible": True,          # toggled during sparse phase
                "alpha": 255,             # fade in/out smoothly
            })

    def update(self):
        # ── Conveyor scroll ──
        self.scroll_offset = (self.scroll_offset + self.scroll_speed) % HEIGHT
        for a in self.asteroids:
            a["y"] = (a["base_y"] + self.scroll_offset) % HEIGHT

        # ── Density cycle ──
        self.density_timer = (self.density_timer + 1) % BELT_DENSITY_CYCLE
        # Use a sine wave: dense at peak, sparse at trough
        density = (math.sin(2 * math.pi * self.density_timer / BELT_DENSITY_CYCLE) + 1) / 2
        # density goes 0.0 (sparse) → 1.0 (dense)
        for i, a in enumerate(self.asteroids):
            # Stagger which asteroids disappear so gaps open naturally
            threshold = (i / len(self.asteroids))
            target_alpha = 255 if density > threshold else 0
            # Smooth fade
            a["alpha"] = clamp(a["alpha"] + (8 if target_alpha > a["alpha"] else -8), 0, 255)
            a["visible"] = a["alpha"] > 10
    ...
    def draw(self, surf):
        for a in self.asteroids:
            ...
            if a["alpha"] < 255:
                # Draw with fade
                s = pygame.Surface((a["r"]*2+2, a["r"]*2+2), pygame.SRCALPHA)
                pygame.draw.circle(s, (*a["col"], a["alpha"]), (a["r"]+1, a["r"]+1), a["r"])
                pygame.draw.circle(s, (80, 50, 30, a["alpha"]), (a["r"]+1, a["r"]+1), a["r"], 1)
                surf.blit(s, (a["x"] - a["r"] - 1, a["y"] - a["r"] - 1))
            else:
                pygame.draw.circle(surf, a["col"], (a["x"], a["y"]), a["r"])
                pygame.draw.circle(surf, (80, 50, 30), (a["x"], a["y"]), a["r"], 1)

    def collides_with_ball(self, ball):
        """Bounce ball off asteroids, or punch through if moving fast enough."""
        hit  = False
        speed = math.hypot(ball.vx, ball.vy)
        punching = speed >= BELT_PUNCH_SPEED  # fast enough to punch through?
        ...
        for _ in range(3):
            for a in self.asteroids:
                ...
                if dist < min_dist and dist > 0:
                    ...
                    if punching:
                        # ── Punch through — push asteroid UP or DOWN only, never sideways ──
                        # Using only ny (vertical component) so asteroids never drift toward black holes
                        push = (min_dist - dist + 4) * 2
                        a["base_y"] += ny * push  # vertical push only
                        # Slight speed loss from punching through
                        ball.vx *= 0.92
                        ball.vy *= 0.92
                    else:
...
class BlackHole:
    def __init__(self, x, y, ring_colour):
        ...
        self.vy    = 0.4
        self.y_min = HEIGHT * 0.2
        self.y_max = HEIGHT * 0.8
    def update(self, mirror=None):
        self.y += self.vy
        if self.y <= self.y_min or self.y >= self.y_max:
            self.vy *= -1
        if mirror:
            mirror.y = HEIGHT - self.y

    def apply_gravity(self, ball):
        """Newton's law of gravitation:  F = G*M / r^2  (applied as acceleration)."""
        dx = self.x - ball.x
        dy = self.y - ball.y
        dist_sq = dx*dx + dy*dy
    ...
    def eject_ball(self, ball):
        ...
        eject_dist = exit_hole.radius + ball.radius + 70  # eject far enough to avoid immediate re-collision
        ...
        exit_speed = max(15.0, math.hypot(ball.vx, ball.vy)) * BH_SPEED_REDUCTION # minimum exit speed to prevent stalling in wormhole
...
class Game:
    def __init__(self, screen, two_player=True, difficulty=None):
        ...
        _font        = "game_font/blockyahh.ttf"
        self.font_lg = pygame.font.Font(_font, 60)
        self.font_md = pygame.font.Font(_font, 28)
        self.font_sm = pygame.font.Font(_font, 18)
        self.stars   = Stars()
        self.belt    = AsteroidBelt(difficulty=difficulty)
        # Two wormhole-connected black holes, one per side
        # Screen is 1200px wide. Belt is at centre (600px).
        # To move closer to belt, increase bh_l X / decrease bh_r X.
        # Current positions: 350px (left) and 850px (right) — each 50px closer than original.
        self.bh_l = BlackHole(350, HEIGHT // 2, (60, 180, 255))  # left  — blue
        self.bh_r = BlackHole(850, HEIGHT // 2, (180, 60, 255))  # right — purple
    ...
    def score_point(self, scorer, direction):
        ...
        if scorer.score >= WINNING_SCORE:
            self.game_over = True
            self.winner = "Terrestrials" if scorer is self.paddle_l else "Invaders"
        else:
    ...
    def update_ai(self):
        cfg = AI_SETTINGS.get(self.difficulty, AI_SETTINGS["medium"])
        self.ai_delay += 1
        if self.ai_delay >= cfg["delay"]:
            self.ai_delay = 0
            error = random.uniform(-cfg["error"], cfg["error"])
            self.ai_target_y = self.ball.y + error
        diff = self.ai_target_y - self.paddle_r.y
        move = clamp(diff, -cfg["speed"], cfg["speed"])
        self.paddle_r.y = clamp(self.paddle_r.y + move,
                                self.paddle_r.H // 2,
                                HEIGHT - self.paddle_r.H // 2)
    ...
    def update(self):
        ...
        self.ball.update()
        self.belt.update()  # scroll + density cycle
        self.bh_l.update(mirror=self.bh_r)
...
class Menu:
    ...
    def __init__(self, screen):
        ...
        self.clock    = pygame.time.Clock()
        _font        = "game_font/blockyahh.ttf"
        self.font_xl  = pygame.font.Font(_font, 48)
        self.font_lg  = pygame.font.Font(_font, 20)
        self.font_sm  = pygame.font.Font(_font, 12)
        self.current  = self.SCREEN_MAIN
        self._build_buttons()
        self.alien_closed = pygame.image.load("sprite/Acloesed_mouth.png").convert_alpha()
        self.alien_open   = pygame.image.load("sprite/Aopen_mouth.png").convert_alpha()
    ...
    def _draw_title(self):
        draw_glow(self.screen, (0, 150, 255), (WIDTH//2, 120), 60, alpha=60)
        title = self.font_xl.render("SPACE COMMAND", True, C_WHITE)
        self.screen.blit(title, (WIDTH//2 - title.get_width()//2, 80))
        sub = self.font_sm.render("electronic tennis for spacefaring civilization", True, C_HUD)
        self.screen.blit(sub, (WIDTH//2 - sub.get_width()//2, 145))

    def _draw_main(self, mp):
        self._draw_stars()
        self._draw_title()
        prompt = self.font_lg.render("SELECT MISSION", True, C_HUD)
        self.screen.blit(prompt, (WIDTH//2 - prompt.get_width()//2, HEIGHT//2 - 95))
        self.btn_1p.draw(self.screen, self.font_lg)
        self.btn_2p.draw(self.screen, self.font_lg)
        # Bottom text box
        hint_text = "P1: W / S     P2: ↑ / ↓     |     Hold D for Dev Mode"
        box_w, box_h = 700, 45
        box_x = WIDTH//2 - box_w//2
        box_y = HEIGHT - 60
        box_surf = pygame.Surface((box_w, box_h), pygame.SRCALPHA)
        box_surf.fill((120, 120, 160, 100))
        self.screen.blit(box_surf, (box_x, box_y))
        pygame.draw.rect(self.screen, (120, 120, 160), (box_x, box_y, box_w, box_h), 2, border_radius=8)
        hint = self.font_sm.render(hint_text, True, C_WHITE)
        self.screen.blit(hint, (WIDTH//2 - hint.get_width()//2, box_y + box_h//2 - hint.get_height()//2))
        # Alien
        any_hovered = self.btn_1p.hovered or self.btn_2p.hovered
        alien = self.alien_open if any_hovered else self.alien_closed
        alien_small = pygame.transform.scale(alien, (210, 210))
        self.screen.blit(alien_small, (WIDTH - 220, HEIGHT - 220))

    def _draw_difficulty(self, mp):
        self._draw_stars()
        self._draw_title()
        prompt = self.font_lg.render("SELECT DIFFICULTY", True, C_HUD)
        self.screen.blit(prompt, (WIDTH//2 - prompt.get_width()//2, HEIGHT//2 - 140))
        descs = [
        ...
        active_desc = "hover a difficulty to learn more"
        ...
        self.screen.blit(box_surf, (box_x, box_y))
        pygame.draw.rect(self.screen, (120, 120, 160), (box_x, box_y, box_w, box_h), 2, border_radius=8)
        d = self.font_sm.render(active_desc, True, C_WHITE)
        self.screen.blit(d, (WIDTH//2 - d.get_width()//2, box_y + box_h//2 - d.get_height()//2))
        # Alien
        any_hovered = self.btn_easy.hovered or self.btn_medium.hovered or self.btn_hard.hovered
        alien = self.alien_open if any_hovered else self.alien_closed
        alien_small = pygame.transform.scale(alien, (210, 210))
        self.screen.blit(alien_small, (WIDTH - 220, HEIGHT - 220))
if __name__ == "__main__":
    try:
        ...
        pygame.display.set_caption("2001: A Space paddle")
        Menu(screen).run()
