...
G_CONSTANT   = 180_000   # gravitational constant (tuned for fun)
...
class BlackHole:
    ...
    def apply_gravity(self, ball):
        ...
        if dist < 5:
            return
        # a = G*M / r²   (ball mass cancels – same as gravitational acceleration)
        accel = self.mass / dist_sq
        # Decompose into x/y
...
class Game:
    ...
    def update(self):
        ...
        if self.bh.swallowed(self.ball):
            self.spawn_particles(self.ball.x, self.ball.y, (180, 60, 255), 25)
            self.ball.x = self.bh.x - (self.ball.x - self.bh.x)
            self.ball.y = self.bh.y - (self.ball.y - self.bh.y)
            self.ball.vx *= -0.8
            self.ball.vy *= -0.8
